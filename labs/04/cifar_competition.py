    ...
    def __init__(self,in_channels,double=False):
        ...
        if not double:
            ...
            self.resiudal_block_second_part = torch.nn.Sequential(torch.nn.Conv2d(in_channels,in_channels//2,kernel_size=1,stride=2,padding=1,bias=False),
                                                                  ...
                                                                  torch.nn.BatchNorm2d(in_channels//2),
                                                                  torch.nn.ReLU(),
                                                                  torch.nn.Conv2d(in_channels//2,2*in_channels,1,1,padding='same'))

            self.residual_connection = torch.nn.Sequential(torch.nn.Conv2d(in_channels,2*in_channels,kernel_size=1,stride=2,padding=1))
...
def main(args: argparse.Namespace) -> None:
    ...
    print(model)
    _optimizer = torch.optim.AdamW(model.parameters(),lr=0.001,weight_decay=0.001)

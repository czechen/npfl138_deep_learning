...
class Model(npfl138.TrainableModule):
    def __init__(self, args: argparse.Namespace, train: MorphoDataset.Dataset) -> None:
        ...
        A = torch.ones(len(tags),len(tags))
        for i in range(len(tags)):
            for j in range(len(tags)):
                if tags[j][0] == 'O' or tags[j][0] == 'B': # Non I-TYPE tags can follow anything
                    continue
                else:
                    if tags[j][-1] == tags[i][-1] and tags[i] != 'O': #i and j tags have the same type and i tag is not O
                        continue
                    else:
                        A[i][j] = 0
    ...
    def forward(self, word_ids: torch.Tensor) -> torch.Tensor:
        ...
        hidden,lengths = torch.nn.utils.rnn.pad_packed_sequence(packed,batch_first=True)
    ...
    def constrained_decoding(self, logits: torch.Tensor, word_ids: torch.Tensor) -> torch.Tensor:
        ...
        batch_size, num_tags, seq_len = logits.size()
        
        alpha = torch.full((batch_size, seq_len, num_tags), -float('inf'))
        beta = torch.zeros((batch_size, seq_len, num_tags), dtype=torch.long)
        padding_mask = (word_ids != MorphoDataset.PAD)  
        A_log = torch.log(self._A) 
        O_tag_idx = num_tags - 1          

        valid_starts = self._A[O_tag_idx] > 0
        alpha[:, 0, valid_starts] = logits[:, valid_starts, 0]
        for t in range(1, seq_len):
            best_scores,beta[:, t, :] = torch.max(alpha[:, t-1, :].unsqueeze(2) + A_log.unsqueeze(0), dim=1)
            alpha[:, t, :] = best_scores + logits[:, :, t]

        predictions = torch.zeros((batch_size, seq_len), dtype=torch.long)
        predictions[:, -1] = torch.argmax(alpha[:, -1, :], dim=1)

        for t in range(seq_len - 2, -1, -1):
            batch_indices = torch.arange(batch_size)
            predictions[:, t] = beta[batch_indices, t+1, predictions[:, t+1]]        

        pad_tensor = torch.full_like(predictions, MorphoDataset.PAD)
        predictions = torch.where(padding_mask, predictions, pad_tensor)
        return predictions
...
def main(args: argparse.Namespace) -> dict[str, float]:
    ...
    model.configure(
        ...
        metrics={
            ...
            "f1_greedy": npfl138.metrics.BIOEncodingF1Score(labels=list(morpho.train.tags.string_vocab),ignore_index=morpho.PAD)
        },
        logdir=args.logdir,
    )
